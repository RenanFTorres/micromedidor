...
void app_main(void)
{
   ...
   uint32_t corrente[NO_OF_VALUES] = {};
    ...
    for (int j = 0; j < NO_OF_VALUES; j++) {
        ...
        //CORRENTE ELETRICA
        // A estrutura do código segue a mesma lógica da aquisição da tensão
    ...
    //    corrente[j] = adc_reading2;

        // Printing counting time

        /* Print the timer values as visible by this task */
        
    } // Fim do loop ADC para aquisição de dados de tensão e corrente
        
    
    // Loop para conversão dos valores digitais da etapa anterior em mV e mA (os valores em mA na verdade são dados de tensão proporcionais aos valores em mA)
    for (int j = 0; j < NO_OF_VALUES; j++) {
        ...
        float current = esp_adc_cal_raw_to_voltage(corrente[j], adc_chars);
        current -= 1555;
        current *= 0.0041;
        //corrente -= 1585;
        //corrente *= 0.0041;
        corrente_rms += current*current;
        somatorio += abs(voltage*current);
        //printf("Raw: %d\tCorrente: %f\n", adc_reading2, corrente);
        //vTaskDelay(pdMS_TO_TICKS(12));
        
        
        } // Fim do loop de converão em mV e mA
