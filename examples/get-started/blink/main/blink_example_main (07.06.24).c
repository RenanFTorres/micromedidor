    ...
    //fflush(stdout);
        ...
        if (flag == 1 && flag2 == NO_OF_VALUES_ADS) {
            ...
            for (int s = 0; s < NO_OF_VALUES_ADS; s++) {
                ...
                corrente_rms += (vetor_corrente[s]-media_i)*(vetor_corrente[s]-media_i);
            ...
            voltage_rms *= 619; //// Relação de transformação da tensão (220V/0,355V). 220V - Valor real (multímetro); 0,355V valor medido na saída do ZMPT pelo osciloscópio
            
            corrente_rms = 1.408*sqrt(corrente_rms/NO_OF_VALUES_ADS); 
            ...
            //// 5 - Fator multiplicativo pelo fato do sensor de corrente ser 5A/1V. /4 - O valor coletado da corrente é 4x o valor real, pois o cabo dá 4 voltas no sensor.
