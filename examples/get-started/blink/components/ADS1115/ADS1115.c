...
static ads1115_t *ads_cfg;  // Ponteiro para uma struct tipo ads_1115_t
...
extern inline esp_err_t ADS1115_request_continuous_ended_AIN1();
...
esp_err_t ADS1115_request_by_definition(uint8_t def)  // Recebe variável de 8bits que configura o MUX das entradas 
{
 //   int16_t def2 =  def + 0x00FF;
    ads_cfg->reg_cfg |= (def << 8) & 0xF000;


    //ads_cfg->reg_cfg &= ADS1115_CFG_MS_MUX_OMASK;     // "ads_cfg->reg_cfg" - Através do ponteiro "ads_cfg" o comando "->"" acessa o campo "reg_cfg"
    //ads_cfg->reg_cfg |= (def << 8) & 0xFF00;          // C |= 2	é o mesmo que C = C | 2
